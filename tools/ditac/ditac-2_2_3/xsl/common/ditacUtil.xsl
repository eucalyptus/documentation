...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="u:fmbmTOCEntryId" as="xs:string">
    <xsl:param name="entry" as="element()"/>

    <xsl:choose>
      <xsl:when test="$entry/@id = '__AUTO__'">
        <xsl:variable name="role" select="string($entry/@role)"/>
        <xsl:choose>
  ...
  <xsl:function name="u:fmbmTOCEntryTitle" as="xs:string">
    <xsl:param name="entry" as="element()"/>

    <xsl:choose>
      <xsl:when test="$entry/@title = '__AUTO__'">
        <xsl:variable name="role" select="string($entry/@role)"/>
        <xsl:sequence select="u:localize($role, $entry)"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:sequence select="$entry/@title"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
      ...
      <xsl:when test="exists($topicEntry)">
        ...
        <xsl:choose>
          <xsl:when test="$role = 'part' or
                          ...
                          $role = 'section8' or
                          $role = 'section9' or
                          $role = 'frontmattersection' or
                          $role = 'backmattersection' or
                          $role = 'amendments' or
                          $role = 'bookabstract' or
                          $role = 'colophon' or
                          $role = 'dedication' or
                          $role = 'draftintro' or
                          $role = 'preface' or
